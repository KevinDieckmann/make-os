    ...
    <div className="os-card" style={{ padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontFamily: 'var(--mono-font)', fontSize: 8.5, color: connected ? '#00ff66' : '#ff8800', letterSpacing: '0.08em' }}>
        ...
        {WHOOP_METRICS.map(m => {
          ...
          return (
            <div key={m.key} style={{ background: '#0a0a0a', border: '1px solid #161616', borderLeft: `2px solid ${m.accent}`, padding: '10px 12px' }}>
              <div style={{ fontFamily: 'var(--mono-font)', fontSize: 8.5, color: m.accent, letterSpacing: '0.08em', marginBottom: 8 }}>
                {m.label.toUpperCase()}
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 3, marginBottom: 6 }}>
                ...
                <span style={{ fontFamily: 'var(--mono-font)', fontSize: 10, color: '#555' }}>{m.unit}</span>
              </div>
              <div style={{ fontFamily: 'var(--mono-font)', fontSize: 8, color: '#666', letterSpacing: '0.02em', lineHeight: 1.3 }}>
                Ziel: {m.target}
              </div>
            </div>
          );
      ...
      {!connected && (
        <div style={{ marginTop: 12, padding: '10px 12px', background: 'rgba(255,136,0,0.06)', border: '1px solid rgba(255,136,0,0.25)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
          <span style={{ fontFamily: 'var(--mono-font)', fontSize: 9.5, color: '#ffb060', letterSpacing: '0.02em', lineHeight: 1.4 }}>
          ...
          <span style={{ fontFamily: 'var(--mono-font)', fontSize: 9, color: '#ff8800', border: '1px solid rgba(255,136,0,0.4)', padding: '5px 10px', whiteSpace: 'nowrap' }}>
            WHOOP VERBINDEN
          </span>
        </div>
      )}
