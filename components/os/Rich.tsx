...
export function renderInline(s: string, key: string): JSX.Element[] {
  return s.split(/\*\*/).map((p, i) => i % 2 === 1
    ? <b key={`${key}-${i}`} style={{ color: T.ink, fontWeight: 600 }}>{p}</b>
...
export function Rich({ text }: { text: string }): JSX.Element {
  ...
  const flush = () => {
    ...
    const liStyle = { fontSize: 14, lineHeight: 1.55, color: T.inkDim };
    blocks.push(ordered
  ...
  lines.forEach((raw, idx) => {
    ...
    flush();
    if (hm) { blocks.push(<div key={`h${idx}`} style={{ fontSize: 13.5, fontWeight: 700, color: T.ink, margin: '10px 0 2px' }}>{renderInline(hm[1], `h${idx}`)}</div>); return; }
    blocks.push(<p key={`p${idx}`} style={{ fontSize: 14, lineHeight: 1.6, color: T.inkDim, margin: '4px 0' }}>{renderInline(line, `p${idx}`)}</p>);
  });
  flush();
  return <>{blocks}</>;
}
