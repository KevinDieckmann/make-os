...
const tage = (n: number, bis = new Date()) => Array.from({ length: n }, (_, i) => { const d = new Date(bis); d.setDate(d.getDate() - (n - 1 - i)); return localDay(d); });
...
function WerkzeugGesundheit() {
  ...
  useEffect(() => {
    fetch('/api/state/vitals').then(r => r.json()).then(d => setVit(d.log ?? {})).catch(() => {});
    fetch('/api/state/health').then(r => r.json()).then(d => setLog(d.log ?? {})).catch(() => {});
  }, []);
  const t14 = tage(14);
  const werte = t14.map(d => ({ d, rec: vit[d]?.rec, sleep: vit[d]?.sleep, routinen: log[d]?.length ?? 0 }));
  const hatWerte = werte.some(w => w.rec != null);
  const streak = (() => { let s = 0; for (let i = t14.length - 1; i >= 0; i--) { if ((log[t14[i]]?.length ?? 0) >= 4) s++; else break; } return s; })();
    ...
    <>
      <div style={{ ...panel, padding: '16px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
          ...
          <Link href="/os/gesundheit" style={{ fontFamily: T.mono, fontSize: 11, color: T.accentInk, textDecoration: 'none' }}>Morgen-Check ›</Link>
        </div>
        {!hatWerte ? (
          ...
          <div style={{ display: 'flex', gap: 3, alignItems: 'flex-end', height: 78, marginTop: 12 }}>
            {werte.map(w => (
              <div key={w.d} title={`${w.d}: Recovery ${w.rec ?? '—'}, Schlaf ${w.sleep ?? '—'}h`} style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', gap: 2, height: '100%' }}>
                ...
                <div style={{ height: `${Math.min(100, ((w.sleep ?? 0) / 9) * 100) * 0.45}%`, background: T.accentInk, opacity: w.sleep == null ? 0.1 : 0.45, borderRadius: '0 0 2px 2px', minHeight: 2 }} />
              </div>
            ))}
          </div>
        )}
        <div style={{ fontFamily: T.mono, fontSize: 10, color: T.muted, marginTop: 8 }}>oben Recovery · unten Schlaf</div>
      </div>

      <div style={{ ...panel, padding: '16px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          <div style={lbl}>Routinen</div>
          <span style={{ fontFamily: T.mono, fontSize: 11, color: streak > 0 ? T.accent : T.muted }}>{streak} Tage in Folge</span>
        </div>
        <div style={{ display: 'flex', gap: 4, marginTop: 12 }}>
          {t14.map(d => {
            const n = log[d]?.length ?? 0;
            return <div key={d} title={`${d}: ${n}/${ROUTINE_ITEMS.length}`} style={{ flex: 1, height: 26, borderRadius: 4, background: n === 0 ? 'rgba(255,255,255,.05)' : T.accent, opacity: n === 0 ? 1 : 0.25 + (n / ROUTINE_ITEMS.length) * 0.75 }} />;
          })}
        </div>
        <div style={{ fontSize: 12.5, color: T.inkDim, marginTop: 10, lineHeight: 1.5 }}>
          Vier von {ROUTINE_ITEMS.length} Häkchen zählen als gehaltener Tag. <Link href="/os/gesundheit" style={{ color: T.accentInk, textDecoration: 'none' }}>Heute abhaken ›</Link>
        </div>
      </div>
    </>
  );
...
function WerkzeugBusiness() {
  ...
  useEffect(() => {
    fetch('/api/state/prospects').then(r => r.json()).then(d => setPs(d.state?.prospects ?? [])).catch(() => {});
    fetch('/api/state/finance').then(r => r.json()).then(d => setFin(d.state ?? null)).catch(() => {});
    ...
    fetch('/api/state/meilensteine').then(r => r.json()).then(d => setMs((d.meilensteine ?? []).filter((x: { bereich: string }) => x.bereich === 'business'))).catch(() => {});
  }, []);
  const stufen = [
  ...
  const ist = fin ? fin.months.reduce((s, m) => s + m.umsatz, 0) : 0;

  // ── Der lebende Geld- und Mandats-Stand ──
  ...
  const hotOffen = ps.filter(p => (p.score ?? 0) >= 80 && p.status !== 'kontaktiert' && p.status !== 'verworfen').length;

  // ── Der EINE Hebel — deterministische Regel-Kette, kein Raten ──
  const hebel = (() => {
    ...
    if (geplantR.length && !gestellt.length) return { text: `${geplantR.length} Rechnung${geplantR.length > 1 ? 'en' : ''} in Vorbereitung — stellen, sonst fließt nichts.`, href: '/os/finanzen', label: 'Finanzplanung' };
    ...
    if (hotOffen) return { text: `${hotOffen} heiße${hotOffen === 1 ? 'r' : ''} Prospect${hotOffen > 1 ? 's' : ''} (80+) unkontaktiert — der erste Schritt kostet 20 Minuten.`, href: '/os/prospecting', label: 'Zielliste' };
  ...
  return (
    <>
      ...
      <div style={{ ...panel, borderLeft: `3px solid ${T.amber}`, padding: '13px 17px' }}>
        ...
        <div style={{ fontSize: 13.5, color: T.ink, lineHeight: 1.5 }}>{hebel.text} <Link href={hebel.href} style={{ color: T.accentInk, textDecoration: 'none' }}>{hebel.label} ›</Link></div>
      </div>
      ...
      <div style={{ ...panel, padding: '16px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 16px' }}>
          {[
            ...
            <div key={i}>
              <div style={{ fontFamily: T.mono, fontSize: 9, letterSpacing: '.1em', textTransform: 'uppercase', color: T.muted }}>{k.l}</div>
              <div style={{ fontFamily: T.mono, fontSize: 17, fontWeight: 700, color: k.c, marginTop: 3 }}>{k.v}</div>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
          <Link href="/os/finanzen" style={{ fontSize: 11.5, color: T.accentInk, textDecoration: 'none' }}>Finanzplanung ›</Link>
          <Link href="/os/crm" style={{ fontSize: 11.5, color: T.accentInk, textDecoration: 'none' }}>CRM ›</Link>
        </div>
        {naechsterMs && (
          <div style={{ marginTop: 12, borderTop: `1px solid ${T.line}`, paddingTop: 10, fontSize: 12.5, color: T.inkDim }}>
            ...
            <span style={{ fontFamily: T.mono, fontSize: 10.5, color: T.muted }}> {naechsterMs.faellig ? `· ${naechsterMs.faellig.slice(8)}.${naechsterMs.faellig.slice(5, 7)}.` : naechsterMs.zeitfenster ?? ''} · {naechsterMs.fortschritt}%</span>
          </div>
        )}
      </div>
      <div style={{ ...panel, padding: '16px 20px' }}>
        ...
        {top && (
          <div style={{ marginTop: 14, background: T.panel2, border: `1px solid ${T.accent}44`, borderRadius: 10, padding: '11px 14px' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: T.ink }}>{top.company} <span style={{ fontFamily: T.mono, fontSize: 12, color: col(top.score ?? null), marginLeft: 6 }}>{top.score ?? '—'}</span></div>
            <Link href="/os/prospecting" style={{ fontSize: 12.5, color: T.accentInk, textDecoration: 'none', display: 'inline-block', marginTop: 5 }}>In der Zielliste öffnen ›</Link>
          </div>
        )}
      </div>
        ...
        {fin && ist > 0 ? (
          <>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
              ...
              <span style={{ fontSize: 13, color: T.inkDim }}>{eur(ist)} von {eur(fin.zielUmsatz)}</span>
            </div>
          ...
          <div style={{ fontSize: 13, color: T.inkDim, lineHeight: 1.5 }}>
            ...
            <Link href="/os/controlling" style={{ color: T.accentInk, textDecoration: 'none' }}> Zahlen eintragen ›</Link>
          </div>
        )}
      </div>
    </>
...
function WerkzeugPlanung() {
  ...
  const load = () => fetch('/api/state/tasks').then(r => r.json()).then(d => setTasks(d.state?.tasks ?? [])).catch(() => {});
  useEffect(() => { load(); }, []);

  const offen = tasks.filter(t => t.status !== 'done');
  const brennt = offen.filter(t => (t.dueDate && t.dueDate <= today) || t.priority === 'critical')
  ...
  async function setStatus(t: Task, status: string) {
    ...
    setTasks(next);
    try {
      const cur = await (await fetch('/api/state/tasks')).json();
  ...
  return (
    <div style={{ ...panel, padding: '16px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
        ...
        <Link href="/os/aufgaben" style={{ fontFamily: T.mono, fontSize: 11, color: T.accentInk, textDecoration: 'none' }}>alle Aufgaben ›</Link>
      </div>
      {!brennt.length ? (
              ...
              <div key={t.id} style={{ display: 'flex', gap: 10, alignItems: 'center', paddingBottom: 8, borderBottom: `1px solid ${T.lineSoft}` }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontFamily: T.mono, fontSize: 10, color: spaet ? T.crit : T.muted, marginTop: 2 }}>
                    {t.priority}{t.dueDate ? ` · ${spaet ? 'überfällig seit' : 'fällig'} ${t.dueDate}` : ''}
                  </div>
                </div>
                <button onClick={() => setStatus(t, t.status === 'in-progress' ? 'todo' : 'in-progress')} disabled={busy === t.id}
                  style={{ fontFamily: T.sans, fontSize: 11.5, fontWeight: 600, padding: '5px 10px', borderRadius: 7, cursor: 'pointer', whiteSpace: 'nowrap', border: `1px solid ${t.status === 'in-progress' ? T.accentInk : T.line}`, background: t.status === 'in-progress' ? `${T.accentInk}22` : 'transparent', color: t.status === 'in-progress' ? T.accentInk : T.inkDim }}>
                  {t.status === 'in-progress' ? 'in Arbeit' : 'anfangen'}
                </button>
                <button onClick={() => setStatus(t, 'done')} disabled={busy === t.id}
                  style={{ fontFamily: T.sans, fontSize: 11.5, fontWeight: 600, padding: '5px 10px', borderRadius: 7, cursor: 'pointer', border: `1px solid ${T.accent}`, background: `${T.accent}22`, color: T.accent }}>
                  ✓
                </button>
              </div>
            );
      ...
      <div style={{ fontSize: 12, color: T.muted, marginTop: 12, lineHeight: 1.5 }}>
        {offen.length} offen — bis 12 gilt als tragbar. Was hier nicht brennt, muss heute nicht in deinen Kopf.
      </div>
    </div>
  );
}
...
function WerkzeugFinanzen() {
  ...
  useEffect(() => { fetch('/api/state/finance').then(r => r.json()).then(d => { setFin(d.state ?? null); if (d.state?.cash) setCash(String(d.state.cash)); }).catch(() => {}); }, []);
  ...
  return (
    <div style={{ ...panel, padding: '16px 20px' }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        <button onClick={speichern} disabled={saving || !fin} style={{ fontFamily: T.sans, fontSize: 12.5, fontWeight: 700, padding: '10px 16px', borderRadius: 9, border: 'none', cursor: saving ? 'default' : 'pointer', background: saving ? T.line : T.accent, color: saving ? T.muted : '#04110F' }}>
          {saving ? '…' : 'Übernehmen'}
        </button>
        <div style={{ flex: 1, minWidth: 160 }}>
          ...
          <div style={{ fontSize: 11.5, color: T.muted, marginTop: 2 }}>
            {burn > 0 ? `bei Ø Burn ${eur(burn)}/Monat` : 'trag Kosten im Controlling ein, dann rechnet es'}
          </div>
        </div>
      </div>
      <div style={{ fontSize: 12.5, color: T.inkDim, marginTop: 14, lineHeight: 1.5, paddingTop: 12, borderTop: `1px solid ${T.lineSoft}` }}>
        ...
        <Link href="/os/controlling" style={{ color: T.accentInk, textDecoration: 'none' }}> Monatszahlen pflegen ›</Link>
      </div>
    </div>
  );
}
...
function WerkzeugSozial() {
  ...
  const [ritLog, setRitLog] = useState<Record<string, string[]>>({});
  // Die Route liefert { journal } — nicht log/state. Vorher blieb die Kurve
  // deshalb immer leer, auch mit Einträgen.
  useEffect(() => {
    fetch('/api/state/journal').then(r => r.json()).then(d => setJournal(d.journal ?? {})).catch(() => {});
    fetch('/api/state/rituale').then(r => r.json()).then(d => setRitLog(d.log ?? {})).catch(() => {});
  }, []);

  const heute = localDay();
  const gehalten = new Set(ritLog[heute] ?? []);
  async function toggleRitual(id: string) {
    const an = !gehalten.has(id);
    // Sofort umschalten, danach bestätigen lassen — sonst fühlt es sich träge an.
    setRitLog(prev => {
    ...
    try {
      const r = await fetch('/api/state/rituale', {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, an, date: heute }),
      });
    ...
    <>
      <div style={{ ...panel, padding: '16px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          ...
          <Link href="/os/journal" style={{ fontFamily: T.mono, fontSize: 11, color: T.accentInk, textDecoration: 'none' }}>Journal ›</Link>
        </div>
        {!hatStimmung ? (
          <div style={{ fontSize: 13, color: T.inkDim, marginTop: 10, lineHeight: 1.5 }}>
            Diese Säule hat noch keine Datenquelle. Ein Journal-Eintrag pro Woche genügt, damit sie mitzählt.
          </div>
        ) : (
          <div style={{ display: 'flex', gap: 4, alignItems: 'flex-end', height: 60, marginTop: 12 }}>
            {t14.map(d => {
              const m = journal[d]?.mood;
              return <div key={d} title={`${d}: ${m ?? '—'}/5`} style={{ flex: 1, height: m ? `${(m / 5) * 100}%` : 3, background: m ? (m >= 4 ? T.accent : m >= 3 ? T.amber : T.crit) : 'rgba(255,255,255,.06)', borderRadius: '3px 3px 0 0' }} />;
            })}
          </div>
        )}
      </div>
            ...
            return (
              <div key={r.id} onClick={() => toggleRitual(r.id)} style={{ display: 'flex', gap: 10, alignItems: 'baseline', cursor: 'pointer' }}>
                ...
                <div>
                  ...
                  <span style={{ fontFamily: T.mono, fontSize: 10.5, color: T.muted, marginLeft: 8 }}>{r.rhythmus}</span>
...
function WerkzeugZusammenhang() {
  ...
  useEffect(() => {
    fetch('/api/state/vitals').then(r => r.json()).then(d => setVit(d.log ?? {})).catch(() => {});
    fetch('/api/state/tasks').then(r => r.json()).then(d => setTasks(d.state?.tasks ?? [])).catch(() => {});
  }, []);
  ...
  const zeilen = t21
    .map(d => ({
      d,
      rec: vit[d]?.rec,
      fertig: tasks.filter(t => t.completedAt?.slice(0, 10) === d).length,
    }))
    .filter(z => z.rec != null);
  ...
  if (zeilen.length < 5) {
    return (
      <div style={{ ...panel, padding: '16px 20px' }}>
        ...
        <div style={{ fontSize: 13, color: T.inkDim, lineHeight: 1.5 }}>
          Ab etwa fünf Tagen mit Morgen-Check zeige ich hier, ob niedrige Recovery und liegengebliebene
          Aufgaben zusammenfallen — der Zusammenhang, den man im Alltag nie sieht.
  ...
  const gS = schnitt(gute), sS = schnitt(schlechte);
  ...
  return (
    <div style={{ ...panel, padding: '16px 20px' }}>
      ...
      <div style={{ fontFamily: T.mono, fontSize: 10, color: T.muted, marginTop: 6 }}>oben erledigt · unten Recovery</div>
      <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${T.lineSoft}`, fontSize: 13, color: T.inkDim, lineHeight: 1.55 }}>
        An Tagen mit Recovery ab 60 wurden im Schnitt <b style={{ color: T.accent }}>{gS.toFixed(1)}</b> Aufgaben fertig,
        darunter <b style={{ color: sS < gS ? T.amber : T.accent }}>{sS.toFixed(1)}</b>.
...
const WERKZEUGE: Record<string, () => JSX.Element> = {
  health: () => (<><WerkzeugGesundheit /><WerkzeugZusammenhang /></>),
...
export function SaeuleView({ keyName }: { keyName: string }) {
  ...
  useEffect(() => { fetch('/api/performance').then(r => r.json()).then(d => setIdx(d.aktuell)).catch(() => {}); }, []);
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.void, color: T.ink, fontFamily: T.sans }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '26px clamp(16px,3vw,36px) 56px' }}>
        <Link href="/os/performance" style={{ fontFamily: T.mono, fontSize: 11, color: T.muted, textDecoration: 'none', display: 'inline-block', marginBottom: 8 }}>‹ Performance-Index</Link>
        <div style={lbl}>{meta.titel}</div>
        <h1 style={{ fontSize: 25, fontWeight: 600, letterSpacing: '-.02em', margin: '6px 0 4px' }}>{meta.claim}</h1>
        <p style={{ fontSize: 13.5, color: T.inkDim, maxWidth: 660, lineHeight: 1.5 }}>{meta.hin}</p>
        ...
        <div style={{ ...panel, borderTop: `2px solid ${col(s?.score ?? null)}`, padding: '18px 22px', margin: '20px 0 14px' }}>
          <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              <div style={{ fontSize: 13, color: T.inkDim, lineHeight: 1.5 }}>
                Gewicht {s ? Math.round(s.gewicht * 100) : '—'}% im Index · Datenbasis <b style={{ color: (s?.abdeckung ?? 0) >= 0.4 ? T.accent : T.amber }}>{s ? Math.round(s.abdeckung * 100) : 0}%</b>
            ...
            <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${T.lineSoft}`, display: 'flex', flexDirection: 'column', gap: 7 }}>
              {s.faktoren.map((f, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'baseline', opacity: f.echt ? 1 : 0.6 }}>
                  <span style={{ fontFamily: T.mono, fontSize: 13, fontWeight: 700, color: f.echt ? col(f.wert) : T.muted, width: 32, textAlign: 'right', flex: '0 0 auto' }}>{f.echt ? f.wert : '—'}</span>
