...
import {
  vorschau, type Firma, type Rechnung, type Zahlung, type Merkposten, type Planposten, type Rhythmus,
...
export function LiquiditaetView() {
  ...
  const v = useMemo(
    () => plan ? vorschau(plan.firmen, plan.rechnungen, plan.zahlungen, plan.merkposten, heute, wochen, optimistisch, posten) : null,
    [plan, posten, heute, wochen, optimistisch],
  );
          ...
          <>
            ...
            <div style={{ ...panel, borderTop: `2px solid ${v.engpass ? T.crit : T.accent}`, padding: '18px 22px', marginBottom: 14 }}>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 12 }}>
                ...
                <button onClick={() => setOptimistisch(!optimistisch)}
                  style={{ marginLeft: 'auto', fontFamily: T.sans, fontSize: 11.5, padding: '4px 11px', borderRadius: 7, cursor: 'pointer', border: `1px solid ${optimistisch ? T.amber : T.line}`, background: optimistisch ? `${T.amber}1c` : 'transparent', color: optimistisch ? T.amber : T.muted }}>
                  {optimistisch ? 'mit Unsicherem' : 'nur Sicheres'}
                </button>
              </div>
              ...
              <div style={{ fontSize: 13, color: v.engpass ? T.crit : T.inkDim, lineHeight: 1.55, marginBottom: 14 }}>
                ...
                {!!v.unsicher && !optimistisch && <> {eur(v.unsicher)} sind als unsicher nicht mitgerechnet.</>}
              </div>
