...
type Bereich = 'heute' | 'kartei' | 'pipeline' | 'kunden' | 'marketing' | 'events' | 'stammdaten';
...
export function CrmSeite() {
  ...
  const zuBereich = (b: string, a?: string) => router.replace(`${pfad}?s=${b}${a ? `&a=${a}` : ''}`);
  ...
  return (
    <Seite titel="CRM" unter={UNTER[bereich]} rechts={<div className="crm-bereiche" style={{ maxWidth: '100%', overflowX: 'auto', scrollbarWidth: 'none' }}><Segmente liste={BEREICHE} aktiv={bereich} onWahl={wechsle} /></div>}>
      ...
      {bereich === 'kartei' && <Kartei api={api} name={name} auswahl={auswahl} setAuswahl={setAuswahl} start={ansicht} />}
