...
// ─── CRM · Marketing — ansprechbar sein, nicht laut ─────────────────────────
// Was Marketing hier steuert: Wie viele Menschen dürfen wir überhaupt
// ansprechen (Einwilligungsbestand je Kanal), wo läuft eine Frist
// (Art. 14), woher kommen Chancen wirklich (Quelle und Selbstauskunft) und
// welche Themen nennen Kunden in Gesprächen (die „Stimme der Kunden“ aus den
// Notizen — Rohstoff für Beiträge). Keine Likes, keine Öffnungsraten.
...
export function Marketing({ api, zuKontakt }: { api: CrmApi; zuKontakt: (id: string) => void }) {
  ...
  const z = useMemo(() => {
    ...
    const stimmen = kontakte.flatMap(k => (k.aktivitaeten ?? []).filter(a => a.notiz?.bedarf).map(a => ({ id: k.id, name: anzeigename(k), am: a.am, bedarf: a.notiz!.bedarf! }))).sort((a, b) => b.am.localeCompare(a.am)).slice(0, 12);
  ...
  return (
    <>
      ...
      <Karte i={1} akzent={z.art14.length ? LEUCHT.kritisch : undefined}>
      ...
      <Karte i={3}>
        ...
        {z.stimmen.length ? (
          <div style={{ display: 'grid', gap: 6 }}>
            ...
            <div style={{ fontSize: 12.5, color: C.inkLeise }}>Themen für Beiträge und Newsletter kommen von hier — echte Probleme mit eigener Einsicht beantworten.</div>
          </div>
