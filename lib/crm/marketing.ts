...
export function marketingKennzahlen(kontakte: Kontakt[], crm: CrmBestand, heute: string): Kpi[] {
  ...
  return [
    ...
    { id: 'marketing_anteil', label: 'Neue Chancen aus Marketing · 90 Tage', wert: anteil, anzeige: anteil === null ? '—' : `${Math.round(anteil * 100)} %`, ampel: anteil === null ? 'grau' : stufe(anteil, 0.25, 0.1), ziel: '≥ 25 %', quelle: neu.length ? `${ausMk} von ${neu.length} neuen Chancen · Quelle Content/Anfrage oder Gespräch aus einem Beitrag` : 'keine neue Chance in 90 Tagen' },
