...
rsync -azn --stats --exclude backup/ --exclude archiv/ --exclude pruefdaten/ .data/ "make@$ZIEL:/srv/make-os/daten/" | grep -E "Number of (regular )?files|Total transferred"
read -r -p "Jetzt wirklich hochladen? (ja/nein) " a
[ "$a" = "ja" ] || { echo "Abgebrochen."; exit 0; }
rsync -az --exclude backup/ --exclude archiv/ --exclude pruefdaten/ .data/ "make@$ZIEL:/srv/make-os/daten/"
