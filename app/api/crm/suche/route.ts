...
export async function GET(req: Request) {
  ...
  const fs = crm.firmen.filter(f => passt([f.name, f.domain, f.branche, f.stadt]))
    .map(f => ({ art: 'firma', id: f.id, titel: f.name, unter: [f.branche, f.stadt, f.rolle === 'kunde' ? 'Kunde' : ''].filter(Boolean).join(' · '), href: `/os/crm?s=firmen&k=${f.id}`, p: punkte(f.name) + (f.rolle === 'kunde' ? 1 : 0) }))
    .sort((a, b) => b.p - a.p).slice(0, 6);
  const ch = crm.chancen.filter(c => passt([c.titel, c.firma])).slice(0, 4).map(c => ({ art: 'chance', id: c.id, titel: c.titel, unter: `Chance · ${c.stufe}`, href: '/os/crm?s=pipeline', p: 1 }));
  const md = crm.mandate.filter(m => passt([m.kunde, m.titel])).slice(0, 4).map(m => ({ art: 'mandat', id: m.id, titel: `${m.kunde} · ${m.titel}`.slice(0, 90), unter: `Mandat · ${m.status}`, href: '/os/crm?s=kunden', p: 1 }));
  const kp = crm.kampagnen.filter(k => passt([k.name])).slice(0, 3).map(k => ({ art: 'kampagne', id: k.id, titel: k.name, unter: `Kampagne · ${k.status}`, href: '/os/crm?s=marketing', p: 1 }));
