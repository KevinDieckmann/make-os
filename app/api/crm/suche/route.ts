...
export async function GET(req: Request) {
  ...
  const ch = crm.chancen.filter(c => passt([c.titel, c.firma])).slice(0, 4).map(c => ({ art: 'chance', id: c.id, titel: c.titel, unter: `Deal · ${c.stufe === 'qualifiziert' ? 'SQL' : c.stufe}`, href: `/os/markttraktion?s=sales&a=pipeline&k=${c.id}`, p: 1 }));
  const md = crm.mandate.filter(m => passt([m.kunde, m.titel])).slice(0, 4).map(m => ({ art: 'mandat', id: m.id, titel: `${m.kunde} · ${m.titel}`.slice(0, 90), unter: `Mandat · ${m.status}`, href: `/os/markttraktion?s=sales&a=kunden&k=${m.id}`, p: 1 }));
